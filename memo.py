...
class MainWindow(QMainWindow):
    ...
    def savebtnEXfile(self):
        ...
        wb_ex = openpyxl.Workbook('%s' %fnameee_ex)
        ws_ex= wb_ex.active
        row_ex = len(ws_ex['A'])
        set_row_ex = row_ex+1
        print(set_row_ex)
        lb_text = self.lbe1.text()
        ws_ex.cell(row= row_ex+1, column= 1).value = lb_text
        ws_ex.cell(row=row_ex+1, column=1).hyperlink = self.fnim
        wb_ex.save('%s'%fnameee_ex)
